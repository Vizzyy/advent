...
for vector in numbers_drawn:
    ...
    if start_coord[0] != end_coord[0] and start_coord[1] != end_coord[1]:
        while delta[0] != 0 or delta[1] != 0:
            coords_in_vector.append([start_coord[0] + delta[0], start_coord[1] + delta[1]])
            delta[0] += 1 * x_direction * -1
            delta[1] += 1 * y_direction * -1
            print(f"diagonal_delta: {delta}")
        coords_in_vector.append(start_coord)
    else:
